    ...
    <footer className="bg-black text-white py-12 md:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto container">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-12 md:gap-24 mb-16 md:mb-32">
          ...
          {/* Contact Info */}
          <div className="w-full md:w-4/12 flex flex-col items-start md:items-end">
            <div className="space-y-6">
              <h3 className="text-lg md:text-xl font-bold font-montserrat text-white md:text-right">
                Contact Info
              </h3>
              <div className="flex items-center gap-6">
                <Link href="mailto:hello@stockus.id" className="text-white hover:text-brand transition-colors">
                  <Mail className="w-6 h-6" />
                </Link>
                <Link href="https://discord.gg/stockus" target="_blank" className="text-white hover:text-brand transition-colors">
                  <DiscordIcon />
                </Link>
                <Link href="https://instagram.com/stockus_id" target="_blank" className="text-white hover:text-brand transition-colors">
                  <Instagram className="w-6 h-6" />
                </Link>
                <Link href="https://wa.me/something" target="_blank" className="text-white hover:text-brand transition-colors">
                  {/* Note: WhatsApp icon SVG path above is complex, using simplified one or lucide equivalent if preferred. 
                      Actually let's use a simpler WhatsApp SVG for now or just the path provided. */}
                  <WhatsAppIcon />
                </Link>
              </div>
            </div>
