...
must_haves:
  truths:
    ...
    - "Authenticated user can initiate workshop payment"
...
- 04-01: Payment schemas (payments table for recording)
...
paymentRoutes.post(
  ...
  async (c) => {
    ...
    // TODO: Fetch workshop from database and get price
    // For now, use placeholder - this will be extended when workshops table is added
    const workshopPrice = 500000 // Placeholder
    const workshopName = `Workshop #${body.workshopId}` // Placeholder
...
- Workshop payment creates pending record and returns Snap token
